...
void editarClientePorNome() {
    ...
    if (!encontrado) {
        printf("Cliente '%s' n�o encontrado!\n", nomeBusca);
    }
...
void eliminarClientePorNome() {
    ...
    if (!encontrado) {
        ...
        remove("TEMP.DAT"); // Exclui o arquivo tempor�rio se nada foi removido
    } else {
        remove("CLIENTES.DAT");  // Remove o original
        rename("TEMP.DAT", "CLIENTES.DAT"); // Renomeia o tempor�rio
    }
}
...
void editarReserva() {
    ...
    while (fread(&reserva, sizeof(RESERVA), 1, arquivo)) {
        if (reserva.idCliente == idClienteBusca) {
            ...
            printf("Novo n�mero de pessoas: ");
            scanf("%d", &reserva.numPessoas);

            printf("Novo hor�rio (hh:mm): ");
            scanf("%s", reserva.horario);

            printf("Novo funcion�rio respons�vel: ");
            getchar(); 
    ...
    if (!encontrado) {
        printf("Reserva para o cliente %d n�o encontrada!\n", idClienteBusca);
    }
...
void editarProdutoPorNome() {
    ...
    if (!encontrado) {
        printf("Produto '%s' n�o encontrado!\n", nomeBusca);
    }
...
void eliminarProdutoPorNome() {
    ...
    if (!encontrado) {
        ...
        rename("TEMP.DAT", "PRODUTOS.DAT"); // Renomeia o tempor�rio
    }
}
